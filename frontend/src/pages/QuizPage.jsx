...
export default function PlayQuizPage() {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        setTimeLeft(res.data.data.timeLimit * 60 || 30);
      } catch (error) {
  ...
  const handleNext = async (skipped = false) => {
    if (!answered && !skipped) return toast.error("Please select an answer");
  ...
  const submitQuiz = async () => {
    ...
    try {
      const res = await axiosInstance.post(`/quiz/${id}/submit`, {
        answers,
        ...
        <div className="quiz-layout">
          ...
          <div className="quiz-left">
            ...
            <div className="footer-row">
              <button className="btn-skip" onClick={() => handleNext(true)}>
                ⚑ Skip
              </button>
              <button
                className="btn-next"
